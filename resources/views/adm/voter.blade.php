<html>
    ...
    <body class="w-100" style="background-color: #EEE;">
        ...
        <div id="container-system">
            @include('adm/header')
            <p class="m-3 p-3 bg-success text-white"><a href="{{ url('/adm/cpanel') }}">CPanel</a> <i class="bi bi-caret-right-fill text-white"></i> <?php if(!empty(@$user->id)){?><a href="{{ route('adm.voters') }}" target="_self">Editar/Excluir Eleitor</a> <i class="bi bi-caret-right-fill text-white"></i> Editar Eleitor - ({{ @$user->name }})<?php }else{ ?>Cadastrar Eleitor<?php } ?></p>
