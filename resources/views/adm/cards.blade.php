<html>
    ...
    <body class="w-100" style="background-color: #EEE;">
        <div class="d-flex justify-content-center">
            <div id="msg-popup" class="animated">
                ...
                <form id="form-delete" name="form-delete" class="btns-delete d-none">
                    @csrf
                    @method('PUT')
                    <input type="hidden" id="route-delete" name="route-delete" value="{{ route('adm.del.card.do') }}">
                    <input type="hidden" id="idDelete" name="idDelete">
                    <button type="button" class="n-card btn btn-success btn-sm">Cancelar</button>
                    <button type="submit" class="y-card btn btn-danger btn-sm">Excluir</button>
                </form>
        ...
        <div id="container-system-adm">
            @include('adm/header')
            <p class="m-3 p-3 bg-success text-white"><a href="cpanel">CPanel</a> <i class="bi bi-caret-right-fill text-white"></i> Editar/Excluir Cédulas</p>
            <div class="m-3 p-4 row bg-white">

                <form id="form-search" name="form-search" autocomplete="off" method="GET" style="padding-left: 0;">

                    <div class="input-group mb-3">
                        <input type="text" id="src" name="src" class="form-control" data-module="card" placeholder="Digite o titulo da cédula a ser buscada">
                ...
                <ul id="container-list" name="container-list" class="list-group">
                    <li class="list-group-item list-group-item-success">
                        ...
                        <div class="col-2 fw-bolder text-success">Localidade</div>
                    </li>
                    ...
                    @else
                        ...
                        @else
                            ...
                            <li id="line-<?=$card->id;?>" class="list-group-item">
                                <div class="col-7 fw-bolder pt-1">{{ @$card->title }}</div>
                                <div class="col-2 fw-bolder pt-1">{{ @$card->location->local }}</div>
                                <div class="container-actions col-3 fw-bolder text-end">
                                    <a type="button" href="{{ route('adm.edit.card', ['card' => $card->id, 'local' => $card->local]) }}" target="_self" class="edit-card btn btn-primary btn-sm text-white"><i class="bi bi-pencil-fill pt-1 me-1 text-white"></i>Editar</a>
                                    <button type="submit" data-id={{ @$card->id }} data-module="card" class="delete btn btn-danger btn-sm"><i class="bi bi-trash-fill pt-1 me-1 text-white"></i>Excluir</button>
                                </div>
                            </li>
